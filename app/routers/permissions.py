...
@user_is_admin
async def toggle_supplier_permission(db: DbSessionDep, curr_user: CurrUserPayloadDep, user_id: int):
...
@user_is_admin
async def toggle_customer_permission(db: DbSessionDep, curr_user: CurrUserPayloadDep, user_id: int):
    user = await get_user(db, user_id)
