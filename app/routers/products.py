...
async def get_all_products(db: DbSessionDep):
    products = await get_objects_or_404(
        db, select(Product).join(Category).where(
...
@user_is_supplier
async def create_product(db: DbSessionDep, product: CreateProduct, curr_user: CurrUserPayloadDep):
    category: Category = await get_object_or_404(
        db, select(Category).where(Category.id == product.category),
...
@user_is_supplier
async def update_product(db: DbSessionDep, product_slug: str, new_product: CreateProduct,
                         curr_user: CurrUserPayloadDep):
    product: Product = await get_object_or_404(
        db, select(Product).where(Product.slug == product_slug),
        'There is no product found'
    )
...
@user_is_supplier
async def delete_product(db: DbSessionDep, product_slug: str, curr_user: CurrUserPayloadDep):
    product = await get_object_or_404(
        db, select(Product).where(Product.slug == product_slug, Product.is_active == True),
        'There is no product found'
    )
